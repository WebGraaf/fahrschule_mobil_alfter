...
export const SplitMediaText1zu1: React.FC<SplitMediaText1zu1Props> = ({
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Container fade in with scale effect
      tl.fromTo(
      ...
      // Image slide in from left or right based on position with rotation
      const imageX = imagePosition === 'left' ? -100 : 100;
      tl.fromTo(
        imageRef.current,
        { opacity: 0, x: imageX, rotationY: imagePosition === 'left' ? -15 : 15 },
        { opacity: 1, x: 0, rotationY: 0, duration: 0.8, ease: 'back.out(1.7)' },
        '-=0.4'
      );

      // Text content staggered reveal with bounce effect
      tl.fromTo(
